...
describe("PromptManager", () => {
  ...
  test("deletePrompts should throw error for invalid indices", () => {
    expect(() => manager.deletePrompts([5])).toThrow("Index 5 is out of bounds.");
    expect(() => manager.deletePrompts([-1])).toThrow("Index -1 is out of bounds.");
  });
  ...
  test("deletePrompts should throw error if indices is not an array", () => {
    expect(() => manager.deletePrompts("invalid")).toThrow("Indices must be an array.");
  });
});
